...
class PlanetDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tp.Dict[str, np.ndarray]:
        ...
        img_path = f"{os.path.join(self.config.images_dir, row.image_name)}.jpg"
        target = np.array(row.drop(["image_name", ]), dtype="float32")
...
def get_class_names(config: Config) -> tp.List[str]:
    class_names = list(pd.read_csv(config.train_dataset_path, nrows=1).drop("image_name", axis=1))
